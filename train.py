...
def loading_2D_data(config):
    image_paths = glob(f"{config.dataset}/images_2ch/*.npy")
    label_paths = glob(f"{config.dataset}/labels_2ch/*.npy")
...
def train(config):
    ...
    # Config name
    config.name = f"2ch_bs{config.batch_size}_ps{config.patch_size}_epo{config.epochs}_hw{config.img_size}"
